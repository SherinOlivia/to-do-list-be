...
const resetPasswordRequest = async (req: Request, res: Response) => {
 ...
 const resetPassword = async (req: Request, res: Response) => {
    try {
        ...
        const user = await DBLocal.promise().query("SELECT * FROM w18MP.users WHERE email = ?", [email]) as RowDataPacket[];
        if (!user) {
            return res.status(400).json(errorHandling(null, "User not found"));
        }
        
        const hashedPassword = await bcrypt.hash(password, 10);
        await DBLocal.promise().query("UPDATE w18MP.users SET password = ? WHERE email = ?", [hashedPassword, email]);
